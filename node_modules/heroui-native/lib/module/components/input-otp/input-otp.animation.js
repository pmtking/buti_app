...
export function useInputOTPSlotCaretAnimation(options) {
  ...
  useEffect(() => {
    ...
    opacity.set(withRepeat(withSequence(withTiming(opacityValue[0], {
      duration: opacityDuration
    }), withTiming(opacityValue[1], {
      duration: opacityDuration
    })), -1, true));
    height.set(withRepeat(withSequence(withTiming(heightValue[0], {
      duration: heightDuration
    }), withTiming(heightValue[1], {
      duration: heightDuration
    })), -1, true));
  }, [isAnimationDisabledValue, opacity, height, opacityValue, opacityDuration, heightValue, heightDuration]);
...
export function useInputOTPSlotValueAnimation(options) {
  ...
  const textEntering = getAnimationValueProperty({
    ...
    property: 'entering',
    defaultValue: FlipInXDown.duration(250).delay(0).easing(Easing.bezier(0, 0.75, 0.5, 0.9).factory()).build()
  });
  const textExiting = getAnimationValueProperty({
    animationValue: animationConfig?.text,
    property: 'exiting',
    defaultValue: FlipOutXDown.duration(250).easing(Easing.bezier(0.6, 0.1, 0.4, 0.8).factory()).build()
  });
  return {
...
//# sourceMappingURL=input-otp.animation.js.map