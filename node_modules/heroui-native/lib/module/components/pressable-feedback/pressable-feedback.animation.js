...
import { Easing, interpolate, useAnimatedStyle, useDerivedValue, useSharedValue, withTiming } from 'react-native-reanimated';
...
export function usePressableFeedbackRootAnimation(options) {
  ...
  const {
    animationConfig: rootConfig,
    isAnimationDisabled: isRootDisabled
  ...
  const {
    isAnimationDisabled: isScaleOwnDisabled
 ...
 */
function computeRippleLayerStyle(containerW, containerH, centerX, centerY, progress, opacityValues, scaleValues) {
  'worklet';
  ...
  return {
    ...
    opacity: interpolate(progress, [0, 1, 2], [opacityValues[0], opacityValues[1], opacityValues[2]]),
    transform: [{
      translateX
    }, {
      translateY
...
export function usePressableFeedbackRippleAnimation(options) {
  ...
  const getAdjustedDuration = useCallback(() => {
    return Math.min(Math.max(rippleProgressBaseDuration * durationCoefficient.get(), rippleProgressMinBaseDuration), rippleProgressBaseDuration * 2);
  ...
  const animationOnTouchStart = useCallback(event => {
    ...
    nextProgress.set(0);
    nextProgress.set(withTiming(1, {
      duration: adjustedDuration
    }));
  }, [getAdjustedDuration, layer0CenterX, layer0CenterY, layer0Progress, layer1CenterX, layer1CenterY, layer1Progress]);
  const animationOnTouchEnd = useCallback(() => {
    const adjustedDuration = getAdjustedDuration();
    const activeProgress = activeLayerRef.current === 0 ? layer0Progress : layer1Progress;
    activeProgress.set(withTiming(2, {
      duration: adjustedDuration
    }));
  ...
  const rLayer0Style = useAnimatedStyle(() => {
    ...
    return computeRippleLayerStyle(containerWidth.get(), containerHeight.get(), layer0CenterX.get(), layer0CenterY.get(), layer0Progress.get(), opacityValue, scaleValue);
  });
  const rLayer1Style = useAnimatedStyle(() => {
    if (isAnimationDisabledValue) {
      return {};
    }
    return computeRippleLayerStyle(containerWidth.get(), containerHeight.get(), layer1CenterX.get(), layer1CenterY.get(), layer1Progress.get(), opacityValue, scaleValue);
  });
  return {
    rLayer0Style,
    rLayer1Style,
