...
import { getAnimationValueMergedConfig, getAnimationValueProperty, getIsAnimationDisabledValue, getRootAnimationState } from "../../helpers/internal/utils/index.js";
// --------------------------------------------------

export const enteringTop = FadeInUp.springify().withInitialValues({
  opacity: 1,
  transform: [{
    translateY: -100
  }]
...
export const enteringBottom = FadeInDown.springify().withInitialValues({
  opacity: 1,
  transform: [{
    translateY: 100
  }]
...
export function useToastRootAnimation(options) {
  ...
  const panGesture = Gesture.Pan().enabled(!isAnimationDisabledValue && isSwipeable).onBegin(() => {
    isDragging.set(true);
    ...
    const shouldDismiss = Math.abs(translationY) > dismissThreshold || Math.abs(velocityY) > velocityThreshold;
    if (placement === 'top') {
  ...
  const rContainerStyle = useAnimatedStyle(() => {
    ...
    return {
      ...
      transform: [{
        translateY: isDragging.get() ? translateY : withTiming(translateY, translateYTimingConfig)
      }, {
