...
import { getAnimationState, getIsAnimationDisabledValue } from "../utils/animation.js";
/**
...
export const usePopupDialogContentAnimation = ({
  ...
  const panGesture = useMemo(() => Gesture.Pan().enabled(isSwipeable && isOpen && !isAnimationDisabledValue).onStart(() => isDragging.set(true)).onUpdate(event => {
  ...
  }), [contentScale, contentScaleAnchor, contentTranslateY, contentTranslateYAnchor, contentY, isOpen, isDragging, isGestureReleaseAnimationRunning, isSwipeable, onOpenChange, progress, progressAnchor, screenHeight, isAnimationDisabledValue, gestureTranslationY]);
