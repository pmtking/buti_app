...
const cssToRNMap = {
  ...
  transitionProperty: value => {
    return {
      transitionProperty: value.replace(/"/g, "").split(",").filter(token => token !== "" && !token.startsWith("--")).map(token => `"${(0, _utils2.toCamelCase)(token)}"`)
    };
  },
  flex: value => {
...
class RN {
  ...
  cssToRN(property, value) {
    ...
    return Object.entries(joinedStyles).filter(([, value2]) => (0, _utils.isDefined)(value2));
  }
